...
def make_conf_mats(gt_label, preds_probs, threshold=0.5):
    ...
    for i in range(len(gt_label)):
        ...
        # TP
        true_pos = np.sum((gt_label[i] == 1) * (pred_label[i] == 1), axis=0).astype(float)
        # TN
        true_neg = np.sum((gt_label[i] == 0) * (pred_label[i] == 0), axis=0).astype(float)
        # FP
        false_pos = np.sum(((gt_label[i] == 0) * (pred_label[i] == 1)), axis=0).astype(float)
        # FN
        false_neg = np.sum(((gt_label[i] == 1) * (pred_label[i] == 0)), axis=0).astype(float)
        image_conf_mat[0] = true_pos
        image_conf_mat[1] = false_pos
        image_conf_mat[2] = false_neg
        image_conf_mat[3] = true_neg
        conf_mats.append(image_conf_mat)
    return conf_mats
