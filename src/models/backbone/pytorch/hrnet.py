...
from typing import Any, List, Iterable, Iterator, Mapping, Optional, overload, Tuple, TypeVar, Union
...
class HighResolutionModule(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        for fuse_layers in self.fuse_layers:
            y = x[0] if i == 0 else fuse_layers[0](x[0])
            #ここまではOK
            
            j = 0
            for fuse_layer in fuse_layers:
                if j == 0:
                    y = y
                else:
                    y = y + fuse_layer(x[j])
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.stage4, pre_stage_channels = self._make_stage(stage4_cfg, num_channels, multi_scale_output=False)
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    ...
                    #NoneだとTorchScriptに変換した際にfor文がskipされるので恒等写像を入れる
