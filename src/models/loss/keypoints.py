...
class AELoss(nn.Module):
    ...
    def singleTagLoss(self, pred_tag, joints):
        ...
        return push/num_tags * 0.5, pull/(num_tags)
...
class MultiLossFactory(nn.Module):
    ...
    def _init_check(self, cfg):
        assert isinstance(cfg.LOSS.WITH_HEATMAPS_LOSS, (list, tuple, omegaconf.listconfig.ListConfig)), \
            'LOSS.WITH_HEATMAPS_LOSS should be a list or tuple'
        assert isinstance(cfg.LOSS.HEATMAPS_LOSS_FACTOR, (list, tuple, omegaconf.listconfig.ListConfig)), \
            'LOSS.HEATMAPS_LOSS_FACTOR should be a list or tuple'
        assert isinstance(cfg.LOSS.WITH_AE_LOSS, (list, tuple, omegaconf.listconfig.ListConfig)), \
            'LOSS.WITH_AE_LOSS should be a list or tuple'
        assert isinstance(cfg.LOSS.PUSH_LOSS_FACTOR, (list, tuple, omegaconf.listconfig.ListConfig)), \
